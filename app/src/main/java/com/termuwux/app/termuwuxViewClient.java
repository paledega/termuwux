...
import com.termuwux.terminal.KeyHandler;
import com.termuwux.terminal.TerminalEmulator;
import com.termuwux.terminal.TerminalSession;
import com.termuwux.view.TerminalViewClient;

import java.util.List;

import androidx.drawerlayout.widget.DrawerLayout;

public final class termuwuxViewClient implements TerminalViewClient {

    final termuwuxActivity mActivity;

    /** Keeping track of the special keys acting as Ctrl and Fn for the soft keyboard and other hardware keys. */
    boolean mVirtualControlKeyDown, mVirtualFnKeyDown;

    public termuwuxViewClient(termuwuxActivity activity) {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent e, TerminalSession currentSession) {
        ...
        if (keyCode == KeyEvent.KEYCODE_ENTER && !currentSession.isRunning()) {
            ...
            if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN || unicodeChar == 'n'/* next */) {
                ...
                // see https://github.com/termuwux/termuwux-api/issues/2
                mActivity.changeFontSize(true);
            } else if (unicodeChar == '-') {
                mActivity.changeFontSize(false);
            } else if (unicodeChar >= '1' && unicodeChar <= '9') {
                int num = unicodeChar - '1';
                termuwuxService service = mActivity.mTermService;
    ...
    public boolean onCodePoint(final int codePoint, boolean ctrlDown, TerminalSession session) {
        if (mVirtualFnKeyDown) {
            ...
            List<termuwuxPreferences.KeyboardShortcut> shortcuts = mActivity.mSettings.shortcuts;
            if (!shortcuts.isEmpty()) {
                int codePointLowerCase = Character.toLowerCase(codePoint);
                for (int i = shortcuts.size() - 1; i >= 0; i--) {
                    termuwuxPreferences.KeyboardShortcut shortcut = shortcuts.get(i);
                    if (codePointLowerCase == shortcut.codePoint) {
                        switch (shortcut.shortcutAction) {
                            case termuwuxPreferences.SHORTCUT_ACTION_CREATE_SESSION:
                                mActivity.addNewSession(false, null);
                                return true;
                            case termuwuxPreferences.SHORTCUT_ACTION_PREVIOUS_SESSION:
                                mActivity.switchToSession(false);
                                return true;
                            case termuwuxPreferences.SHORTCUT_ACTION_NEXT_SESSION:
                                mActivity.switchToSession(true);
                                return true;
                            case termuwuxPreferences.SHORTCUT_ACTION_RENAME_SESSION:
