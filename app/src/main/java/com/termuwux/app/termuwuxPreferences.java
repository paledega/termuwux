...
import static com.termuwux.terminal.EmulatorDebug.LOG_TAG;

final class termuwuxPreferences {
    ...
    termuwuxPreferences(Context context) {
    ...
    static void storeCurrentSession(Context context, TerminalSession session) {
        PreferenceManager.getDefaultSharedPreferences(context).edit().putString(termuwuxPreferences.CURRENT_SESSION_KEY, session.mHandle).apply();
    }

    static TerminalSession getCurrentSession(termuwuxActivity context) {
        String sessionHandle = PreferenceManager.getDefaultSharedPreferences(context).getString(termuwuxPreferences.CURRENT_SESSION_KEY, "");
    ...
    void reloadFromProperties(Context context) {
        ...
        try {
            ...
            Toast.makeText(context, "Could not open properties file termuwux.properties: " + e.getMessage(), Toast.LENGTH_LONG).show();
        ...
        mDefaultWorkingDir = props.getProperty("default-working-directory", termuwuxService.HOME_PATH);
        ...
        if (!workDir.exists() || !workDir.isDirectory()) {
            ...
            mDefaultWorkingDir = termuwuxService.HOME_PATH;
        }
    ...
    private void parseAction(String name, int shortcutAction, Properties props) {
        ...
        if (!(parts.length == 2 && parts[0].trim().equals("ctrl")) || input.isEmpty() || input.length() > 2) {
            Log.e("termuwux", "Keyboard shortcut '" + name + "' is not Ctrl+<something>");
            return;
        }
